...
The Agent SDK includes built-in tracing, collecting a comprehensive record of events during an agent run: LLM generations, tool calls, handoffs, guardrails, and even custom events that occur. Traces stay in the process until you add a [processor](#custom-tracing-processors) that sends them somewhere.
...
-   The `TraceProvider` starts with no processors, so traces and spans go nowhere until you add one. A [`BatchTraceProcessor`][agents.tracing.processors.BatchTraceProcessor] over a [`BackendSpanExporter`][agents.tracing.processors.BackendSpanExporter] posts them in batches to the endpoint you give the exporter.

There are two ways to add processors:

-   [`add_trace_processor()`][agents.tracing.add_trace_processor] adds a processor that receives traces and spans as they are ready.
-   [`set_trace_processors()`][agents.tracing.set_trace_processors] replaces the current processors with the ones you pass.
