...
the ``src/agents`` Web3 tree to confirm signing through a real MPC
cluster instead of placeholder ``return True`` / ``NotImplementedError``
stubs.

This module mirrors the interface of
``sdk/python/hanzo_agents/extensions/web3/mpc_client.py`` (introduced in
PR #4) so that the two parallel Python trees stay in lock-step.
...
class MpcConfig(BaseModel):
    ...
    api_key: str | None = Field(default=None, description="Bearer token, if cluster requires one")
...
class MpcClient:
    ...
    async def _request(self, path: str, payload: dict) -> dict:
        ...
        raise MpcError(
            f"MPC bridge unreachable after {self.config.max_retries + 1} attempts: {last_err}"
        )
    ...
    async def verify(
        ...
        """Ask the cluster to verify a signature.

        Useful for cross-check before trusting a payment ack signed by
        another party.
        """
        if (
            not _looks_like_hex(signature_hex)
            or not _looks_like_hex(digest_hex)
            or not _looks_like_hex(public_key_hex)
        ):
